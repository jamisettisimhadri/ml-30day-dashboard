...
function loadStateFromStorage() {
    const saved = localStorage.getItem("ml_30day_tracker_state");
    if (saved) {
        try {
            const parsed = JSON.parse(saved);
            // Merge with default state structure to handle older profiles
            appState.startDate = parsed.startDate || appState.startDate;
            appState.projects = parsed.projects || appState.projects;
            appState.resources = parsed.resources || appState.resources;
            
            // Loop and merge day details
            DAYS_DATA.forEach(d => {
                if (parsed.days && parsed.days[d.day]) {
                    appState.days[d.day].checked = {
                        learning: !!parsed.days[d.day].checked.learning,
                        coding: !!parsed.days[d.day].checked.coding,
                        revision: !!parsed.days[d.day].checked.revision,
                        project: !!parsed.days[d.day].checked.project
                    };
                    appState.days[d.day].notes = parsed.days[d.day].notes || "";
                }
            });
        } catch (e) {
